...
String getPriorityDirectionText(int id) {
  switch (id) {
    case 0:
      return 'phía trên';
    case 1:
...
class MQTTManager {
  ...
  final String clientId =
      'flutter_traffic_controller_${DateTime.now().millisecondsSinceEpoch}';
  ...
  Future<bool> connect() async {
    try {
      ...
      if (client.connectionStatus?.state == MqttConnectionState.connected) {
        ...
        print('❌ Server connection failed: ${client.connectionStatus}');
        _isConnected = false;
        return false;
      }
    } catch (e) {
  ...
  void disconnect() {
    client.autoReconnect = false; // Ngăn thư viện tự động kết nối lại
    client.disconnect();
    _isConnected = false;
    onDisconnected?.call();
  ...
  Future<bool> reconnect() async {
    ...
    _initClient(); // Khởi tạo lại cấu hình MQTT trước khi kết nối
...
class TrafficLogic extends ChangeNotifier {
  ...
  int _priorityFlashCount = 0;
  Timer? _priorityFlashTimer;
  ...
  void activatePriorityMode(int id) {
    ...
    onShowMessage?.call(
      "🚑 Chế độ ưu tiên: ${getPriorityDirectionText(id)}",
      Colors.red,
    );
    notifyListeners();
  }

  void _startPriorityFlashing() {
    _priorityFlashTimer?.cancel();
    _priorityFlashCount = 0;
    priorityBlink = false;
    _priorityFlashTimer = Timer.periodic(const Duration(milliseconds: 200), (t) {
      priorityBlink = !priorityBlink;
      _priorityFlashCount++;
      notifyListeners();
      if (_priorityFlashCount >= 6) {
        priorityBlink = true;
        t.cancel();
        notifyListeners();
      }
  ...
  void exitPriorityMode() {
    ...
    onShowMessage?.call(
      "✅ Exited priority - Returning to $_modeBeforePriority",
      Colors.blue,
    );
    notifyListeners();
  }
  ...
  void deactivateNightModeManual() {
    ...
    onShowMessage?.call(
      "☀️ Tắt chế độ ban đêm - Quay về chế độ bình thường",
      Colors.amber,
    );
    notifyListeners();
  }
  ...
  void processFirmwareStatus(String message) {
    ...
    if (message.startsWith("STATUS|")) {
      if (message.contains("NIGHT_MODE_AUTO_ON")) {
        ...
        _updateModeFromTimes();
        onShowMessage?.call(
          "🌙 Cảm biến: Trời tối - Vào chế độ ban đêm",
          Colors.indigo,
        );
        notifyListeners();
        ...
        if (id >= 0 && id <= 3) {
          if (!nightMode) {
            ...
            onShowMessage?.call(
              "🚑 Sensor: Priority vehicle ${getPriorityDirectionText(id)}",
              Colors.red,
            );
          } else {
  ...
  void _updateStateMachine() {
    ...
    if (nightMode) {
      ...
      // Do nothing, handled by _priorityFlashTimer
    } else {
      for (int i = 0; i < 4; i++) {
        counters[i]--;
  ...
  bool isRed(int index) =>
      !nightMode && !priorityMode && states[index] == "red";
  bool isYellow(int index) =>
      !nightMode && !priorityMode && states[index] == "yellow";
  bool isGreen(int index) =>
      !nightMode && !priorityMode && states[index] == "green";

  bool isNightYellow() => nightMode && nightBlink;
  bool isPriorityGreen(int index) =>
      priorityMode && index == priorityId && priorityBlink;
  bool isPriorityRed(int index) =>
      priorityMode && index != priorityId && priorityBlink;
...
class _TrafficHomeState extends State<TrafficHome>
  ...
  // BIẾN CỜ MỚI: Dùng để chặn firmware tự động kết nối lại khi đã nhấn Ngắt
  bool _firmwareExplicitlyDisconnected = false;

  String _notificationMessage = "";
  Color _notificationColor = Colors.transparent;
  ...
  void initState() {
    ...
    _mqttManager.onConnected = () {
      if (mounted) {
        ...
        _showSnackBar('✅ Kết nối server thành công', Colors.green);
    ...
    _mqttManager.onDisconnected = () {
      if (mounted) {
        ...
        _showSnackBar('🔌 Mất kết nối server', Colors.orange);
      }
    };
    ...
    _mqttManager.onMessageReceived = (message) {
      ...
      if (message.startsWith("ONLINE")) {
        // Chỉ kết nối lại nếu người dùng không chủ động ngắt
        if (!_firmwareConnected && !_firmwareExplicitlyDisconnected) {
          setState(() {
    ...
    _mqttManager.onStatusReceived = (json) {
      _lastFirmwareMessage = DateTime.now();
      if (!_firmwareExplicitlyDisconnected) {
        _logic.updateFromJson(json);
      }
    };

    // Tự động kết nối MQTT khi khởi chạy
    _connectMQTT();
    ...
    ).animate(CurvedAnimation(parent: _skyController, curve: Curves.easeInOut));
  ...
  void _checkFirmwareConnection() {
    ...
    // NGĂN CHẶN: Nếu người dùng đã nhấn ngắt kết nối, bỏ qua việc kiểm tra timeout để tự bật lại
    if (_firmwareExplicitlyDisconnected) {
      return;
    }
    ...
    if (_firmwareConnected && difference > _firmwareTimeout) {
      ...
      _logic.setSyncedWithFirmware(false);
      _showSnackBar('⏹️ Mất kết nối với sa bàn', Colors.orange, duration: 1);
  ...
  void _syncCurrentStateToFirmware() {
    ...
    if (_logic.nightMode) {
      if (_logic.nightModeManual) {
        ...
        if (_logic.redTime == 15 &&
            _logic.greenTime == 10 &&
            _logic.yellowTime == 5) {
          _mqttManager.publishCommand('N');
        } else if (_logic.redTime == 30 &&
            _logic.greenTime == 25 &&
            _logic.yellowTime == 5) {
          _mqttManager.publishCommand('P');
        } else if (_logic.redTime == 12 &&
            _logic.greenTime == 10 &&
            _logic.yellowTime == 2) {
          _mqttManager.publishCommand('L');
        } else {
          _mqttManager.publishCommand(
            'R${_logic.redTime}G${_logic.greenTime}Y${_logic.yellowTime}',
          );
        }
      }
  ...
  void _showSnackBar(String message, Color color, {int duration = 2}) {
    ...
    _notificationTimer = Timer(Duration(seconds: duration + 1), () {
      if (mounted) {
        setState(() {
  ...
  // CẬP NHẬT: Dialog quản lý kết nối MQTT/Sa bàn
  void showConnectionManagementDialog() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Row(
          children: [
            const Icon(Icons.settings_remote, color: Colors.blue),
        ...
        content: Column(
          ...
          children: [
            ...
            ListTile(
              ...
              leading: Icon(
                _firmwareConnected ? Icons.router : Icons.phonelink_erase,
                color: _firmwareConnected ? Colors.green : Colors.orange,
              ),
              title: const Text("Sa bàn (Phần cứng)"),
              subtitle: Text(_firmwareConnected ? "Đã trực tuyến" : "Đang chờ tín hiệu..."),
              trailing: ElevatedButton(
                style: ElevatedButton.styleFrom(
                  backgroundColor: _firmwareConnected ? Colors.red.shade400 : Colors.green.shade400,
                  foregroundColor: Colors.white,
                ),
                onPressed: () {
                  if (_firmwareConnected) {
                    // XỬ LÝ NGẮT KẾT NỐI: Bật cờ chặn tự kết nối lại
                    setState(() {
                      _firmwareConnected = false;
                      _firmwareExplicitlyDisconnected = true;
                    });
                    _logic.setSyncedWithFirmware(false);
                ...
                child: Text(_firmwareConnected ? "Ngắt" : "Kết nối"),
              ),
            ),
          ],
        ),
          ...
          children: [
            ...
            if (!_mqttConnected)
              Padding(
                ...
                child: Text(
                  '⚠️ Chưa kết nối hệ thống - Chỉ điều khiển local',
                  style: TextStyle(
                    color: Colors.orange,
            ...
            children: [
              if (!_mqttConnected)
                Padding(
                  ...
                  child: Text(
                    '⚠️ Chưa kết nối sa bàn - Chỉ điều khiển local',
                    style: TextStyle(
                      color: Colors.orange,
                      fontSize: getResponsiveFontSize(context, 14),
                    ),
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ElevatedButton(
                        onPressed: () {
                          _logic.activatePriorityMode(0); // UP
                          Navigator.pop(context);
                        },
                        style: ElevatedButton.styleFrom(
                  ...
                  Row(
                    ...
                    children: [
                      ElevatedButton(
                        onPressed: () {
                          _logic.activatePriorityMode(3); // LEFT
                          Navigator.pop(context);
                        },
                        style: ElevatedButton.styleFrom(
                          padding: const EdgeInsets.symmetric(vertical: 12),
                      ...
                      ElevatedButton(
                        onPressed: () {
                          _logic.activatePriorityMode(1); // RIGHT
                          Navigator.pop(context);
                        },
                        style: ElevatedButton.styleFrom(
                          padding: const EdgeInsets.symmetric(vertical: 12),
                  ...
                  Row(
                    ...
                    children: [
                      ElevatedButton(
                        onPressed: () {
                          _logic.activatePriorityMode(2); // DOWN
                          Navigator.pop(context);
                        },
                        style: ElevatedButton.styleFrom(
                          padding: const EdgeInsets.symmetric(vertical: 12),
                            ...
                            child: Row(
                              children: [
                                ...
                                Switch(
                                  ...
                                  activeTrackColor: Colors.indigo.withOpacity(
                                    0.5,
                                  ),
                                ),
                              ],
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildInfoSection(
                            ...
                            content:
                            "🟢 Đã kết nối với sa bàn giao thông.\n"
      ...
      builder: (context, child) {
        return Stack(
          children: [
            ...
            Positioned(
              left:
              MediaQuery.of(context).size.width * _sunMoonPosition.value -
                  30,
            ...
            if (!_logic.nightMode)
              ...List.generate(3, (index) {
                final cloudOffset =
                    (index * 40 + _skyController.value * 200) %
  ...
  Widget buildZebraCrossing({
    ...
    final int stripeCount = (isVertical ? height / 8 : width / 8).floor();
  ...
  Widget buildTrafficLight(int index) {
    bool isRed =
        _logic.isRed(index) ||
            ...
            (_logic.priorityMode &&
                index != _logic.priorityId &&
                _logic.priorityBlink);

    bool isYellow =
        _logic.isYellow(index) ||
            (_logic.nightMode && _logic.nightBlink) ||
            false;

    bool isGreen =
        _logic.isGreen(index) ||
            (_logic.priorityMode &&
                index == _logic.priorityId &&
                _logic.priorityBlink) ||
            false;
            ...
            child: FittedBox(
              ...
              child: Text(
                ...
                style: TextStyle(
                  ...
                  (_logic.nightMode ||
                      _logic.priorityMode ||
                      _logic.isSyncedWithFirmware == false)
                      ? Colors.grey
      ...
      builder: (context, constraints) {
        ...
        return Stack(
          ...
          children: [
            ...
            // Priority Lane Indicators (Moved to the left of each lane)
            Positioned(
              top: (size / 2) - (roadWidth / 2) - (roadWidth * 0.4),
              left: (size / 2) - (roadWidth * 0.35),
              child: const Icon(Icons.arrow_downward, color: Colors.amber, size: 16),
            ),
            Positioned(
              right: (size / 2) - (roadWidth / 2) - (roadWidth * 0.4),
              top: (size / 2) - (roadWidth * 0.35),
              child: const Icon(Icons.arrow_back, color: Colors.amber, size: 16),
            ),
            Positioned(
              bottom: (size / 2) - (roadWidth / 2) - (roadWidth * 0.4),
              left: (size / 2) + (roadWidth * 0.15),
              child: const Icon(Icons.arrow_upward, color: Colors.amber, size: 16),
            ),
            Positioned(
              left: (size / 2) - (roadWidth / 2) - (roadWidth * 0.4),
              top: (size / 2) + (roadWidth * 0.15),
              child: const Icon(Icons.arrow_forward, color: Colors.amber, size: 16),
            ),
  ...
  Widget buildInput(String label, TextEditingController controller) {
    return TextField(
      ...
      decoration: InputDecoration(
        labelText: label,
        labelStyle: TextStyle(fontSize: getResponsiveFontSize(context, 14)),
        ...
        builder: (context, child) {
          return Stack(
            children: [
              ...
              Positioned.fill(
                child: Image.asset(
                  'anh.png', // Assuming 'anh.png' is the grass image
                  fit: BoxFit.cover,
              ...
              SafeArea(
                child: Column(
                  children: [
                    Container(
                      ...
                      child: Column(
                        children: [
                              ...
                              child: Container(
                                ...
                                decoration: BoxDecoration(
                                  gradient: LinearGradient(
                                    colors: _logic.nightMode
                                        ? [
                                      Colors.indigo.shade800.withOpacity(
                                        0.5,
                                      ),
                                      ...
                                      Colors.cyan.shade700.withOpacity(
                                        0.5,
                                      ),
                                    ],
                                  ),
                          ...
                          Row(
                            children: [
                              // CẢI TIẾN: Nhấn vào thanh trạng thái để mở quản lý kết nối
                              Expanded(
                                child: GestureDetector(
                                  onTap: showConnectionManagementDialog,
                                  child: ClipRRect(
                                    borderRadius: BorderRadius.circular(30),
                                    child: BackdropFilter(
                                      ...
                                      child: Container(
                                        ...
                                        child: Row(
                                          ...
                                          children: [
                                            ...
                                            const SizedBox(width: 4),
                                            const Icon(Icons.touch_app, size: 14, color: Colors.black26),
                                            const SizedBox(width: 8),
                                          ],
                                        ),
                                      ),
                    ...
                    // Phần sử dụng Stack để chứa Sa bàn và Thông báo nổi
                    Expanded(
                      child: Stack(
                        alignment: Alignment.topCenter,
                        children: [
                          // 1. Giao diện ngã tư giao thông
                          Padding(
                          ...
                          // 2. Thông báo trượt xuống đè lên trên sa bàn
                          Padding(
                            padding: const EdgeInsets.symmetric(horizontal: 16),
                            child: AnimatedContainer(
                              duration: const Duration(milliseconds: 300),
                              curve: Curves.easeInOut,
                              height: _notificationMessage.isEmpty ? 0 : 40,
                              width: double.infinity,
                              margin: EdgeInsets.only(
                                top: _notificationMessage.isEmpty ? 0 : 8,
                                bottom: _notificationMessage.isEmpty ? 0 : 4,
                              ),
                              decoration: BoxDecoration(
                                color: _notificationColor.withOpacity(0.15),
                                borderRadius: BorderRadius.circular(20),
                                border: Border.all(
                                  color: _notificationColor.withOpacity(0.5),
                                  width: 1.5,
                                ),
                                // Hiệu ứng làm mờ nền phía sau thông báo cho đẹp
                                boxShadow: _notificationMessage.isEmpty
                                    ? []
                                    : [
                                  BoxShadow(
                                    color: Colors.black.withOpacity(0.1),
                              ...
                              child: _notificationMessage.isEmpty
                                  ? null
                                  : ClipRRect(
                                borderRadius: BorderRadius.circular(20),
                                child: BackdropFilter(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          Icon(
                                            ...
                                            color: _logic.nightMode
                                                ? Colors.white
                                                : _notificationColor.withOpacity(
                                              0.8,
                                            ),
                                          ),
                                          const SizedBox(width: 8),
